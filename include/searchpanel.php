<?php
    // Get the current file name and remove the .php extension
    $currentFileName = basename($_SERVER['PHP_SELF'], '.php'); // Get the file name without the extension

    // Ensure the file name contains only valid characters
    $sanitizedFileName = preg_replace('/[^a-zA-Z0-9_]/', '', $currentFileName);

    // Convert the file name to a readable title (capitalize first letters)
    $pageTitle = ucwords(str_replace('_', ' ', $sanitizedFileName));
?>

<section class="search-panel-container">
    <h1><?php echo htmlspecialchars($pageTitle, ENT_QUOTES, 'UTF-8'); ?></h1>
    <form class="search-bar-form" action="search_results.php" method="GET">
        <input 
            type="text" 
            name="query" 
            placeholder="Search..." 
    ...
    <button class="notification-btn" onclick="location.href = '#' ">
